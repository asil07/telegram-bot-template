...
from keyboards.inline.products_keys import build_keyboard
...
async def shipping_options(query: types.ShippingQuery):
    if query.shipping_address.country_code != "UZ":
        await bot.answer_shipping_query(shipping_query_id=query.id,
                                        ok=False,
                                        error_message="Hozircha faqat uzbekistonga")
    elif query.shipping_address.city.lower() == "tashkent" or query.shipping_address.city.lower() == "tosh":
        await bot.answer_shipping_query(shipping_query_id=query.id,
                                        shipping_options=[REGULAR_SHIPPING, FAST_SHIPPING,
                                                          PICKUP_SHIPPING],
                                        ok=True)
    else:
        await bot.answer_shipping_query(shipping_query_id=query.id,
